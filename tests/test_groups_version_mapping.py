...
def test_five_version_controls_use_independent_endpoints_and_payloads():
    ...
    assert "retry-styles/${encodeURIComponent(version)}" in api
    assert "const endpoint = version ?" in api
...
def test_classic_readiness_uses_merge_artifact_fields():
    ...
    assert "if (Number(group.merge_status) === 1) return 1" in view
    assert "return Number(group.classic_status ?? 0)" in view
