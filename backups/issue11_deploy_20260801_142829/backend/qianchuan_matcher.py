...
def _contains_score(haystack: str, needles: Iterable[str], weight: float) -> Tuple[float, List[str]]:
    hits = [n for n in needles if n and n.lower() in haystack]
    return min(1.0, len(hits) / max(1, len(list(needles)))) * weight, hits
...
async def load_group_context(db_path: str, group_id: int, product_id: Optional[str] = None) -> Dict:
    ...
    async with aiosqlite.connect(db_path) as db:
        ...
        if not products:
            ...
            async with db.execute(
                """SELECT * FROM products
                   WHERE enabled = 1 AND (room_id IS NULL OR room_id = ?)
                   ORDER BY CASE WHEN room_id = ? THEN 0 ELSE 1 END, id DESC
                   LIMIT 30""",
                (room_id, room_id),
            ) as cur:
                products = [dict(r) for r in await cur.fetchall()]
