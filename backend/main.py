...
def _is_finished_unsynced_upload_candidate(row) -> bool:
    ...
    if not row["end_time"] or row["end_time"] == row["start_time"]:
        return False
    # Legacy recordings may not have been backfilled with duration metadata.
    # They are still valid finished upload candidates and must match the poller.
    if row["duration_status"] not in (None, "accepted"):
        return False
...
async def set_publish_versions(group_id: int, body: dict):
    ...
    if versions not in allowed:
        raise HTTPException(status_code=400, detail=f"publish_versions must be one of: {', '.join(allowed)}")
    async with aio_connect() as db:
        await db.execute(
...
async def gpu_status():
    ...
    # Use the poller's classifier for the headline too.  Counting every
    # finished row with a file used to disagree with the actual dispatch
    # predicate (for example legacy duration-invalid rows).
    queue_diagnosis = result["transcription_queue"]
    result["pending_transcribe"] = (
        queue_diagnosis.get("can_submit_count", 0)
        + queue_diagnosis.get("counts", {}).get("gpu_job_running", 0)
    )
    ...
    result["poll_state"] = {
        ...
        "last_poll_finished_at": _iso(ps["last_poll_finished_at"]),
        "poll_count": ps["poll_count"],
        "last_poll_error": ps["last_poll_error"],
    ...
    async with aio_connect() as db:
        ...
        async with db.execute(
            """SELECT r.id, r.filename, r.transcribed, r.transcribe_error, r.start_time,
                      ...
                      rm.name as room_name
               FROM recordings r LEFT JOIN rooms rm ON r.room_id = rm.id
               WHERE (r.duration_status IS NULL OR r.duration_status = 'accepted')
