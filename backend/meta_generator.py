...
【通用禁止】节日/节气词（过年/春节/元旦等）；「姐妹」描述中出现 ≤ 1次（标题可出现1次）；禁止「绝绝子/yyds/爱了爱了/好家伙/破防了/buff叠满/OOTD/泰裤辣」等过时/低质网络词；禁止时效性标签；禁止「变美/显年轻X岁/遮住所有缺陷/彻底解决/保证效果」等效果承诺表述（巨量千川低质管控）；禁止「限时/最后X件/仅剩X单/抢完就没了/马上下架」等虚假紧迫感（平台直接降流）；标题字数严格 15~20 字，禁止标点堆叠和无意义符号（❗❗❗/‼️‼️/……等）；Emoji 最多1个，禁止堆叠；不用「绝了/太绝了/真的绝/nb了」等空洞形容词；多用疑问句/场景句/对比句式提升点击率；合理使用具体数字提升可信度（「3个方法/5秒搮定/2步就搞定」）。
【数据/背书禁止（需授权或数据证据，无则违规）】禁止「已销售XX单/卖出X万单/热销X件」等销售数据；禁止「XX博主都在用/明星同款/达人推荐/KOL同款」等背书声称；禁止「粉丝强烈推荐/万人好评/用户评分X分」等未经核实的用户数据；禁止「全网最火/抖音TOP1/同类第一」等最高级排名声称；禁止「官方认证/国家标准/获奖产品」等认证背书（无法核实时）。以上内容如需使用，必须有真实授权书或平台可核实数据，否则一律不写。
