...
【通用禁止】节日/节气词（过年/春节/元旦等）；「姐妹」不超过2次；不用「绝绝子/yyds/爱了爱了」等过时词；禁止时效性标签；禁止「变美/显年轻X岁/遮住所有缺陷/彻底解决/保证效果」等效果承诺表述（巨量千川低质管控）。
【标签要求】每种方案7-10个#标签，覆盖品类词、款式颜色、场景人群、情绪种草；只使用假发/美发品类垂直标签，禁止蹭与假发无关的热点话题（平台会识别为诱骗流量）。
...
注意：禁止「反差/前后对比」切入角度（巨量千川禁止暗示佩戴前后改变外貌）。
禁止开头直接使用「出门五分钟/约会/通勤」等高频套话。
描述（120-180字）：
...
描述（100-150字）：
  ...
  3. 行动引导（1-2句）：「直播间同价，宝子们点左下方小黄车就可以下单」；禁止「最后X单/限时/抓紧」等催单硬广措辞（平台管控卖惨/紧迫感操控）
