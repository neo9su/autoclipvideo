...
_SCORES: dict[str, float] = {
    ...
    '不反光': 9, '颜色自然': 9,
    '特写': 11, '特写镜头': 12, '近景': 9, '近距离': 10, '细节': 11, '发丝质感': 12,
    '放大看': 11, '镜头拉近': 10, '镜头推进': 10,
    '侧面': 10, '从侧面': 10, '侧边': 9, '侧边看': 9, '正面': 8, '背面': 10,
    '后面': 9, '后面看': 10, '左边看': 8, '右边看': 8, '各个角度': 10,
    '整体': 7, '整体造型': 8, '全貌': 7,
    '头顶': 6, '顶部': 5, '俯视': 6,
    '360': 12, '转一圈': 11, '转一下': 9, '转过来': 9,
    # ── 视觉效果词（画面质感）────────────────────────────────────────────────
    '光泽': 8, '反光': 7, '光线': 6, '阳光下': 9, '灯光下': 8,
    '质感': 10, '质地': 9, '摸起来': 9, '手感': 9,
    # ── 身体部位细节（需放大对应区域）──────────────────────────────────────────
    ...
    '送保养视频': 9, '终身护理建议': 10,
    '送防尘袋': 8, '发网': 8, '钢梳': 7,
    # ── 新潮/趋势/限定（novelty & trend signals）────────────────────────────────
    # 平台明确加分：商品自带新潮元素，优先推流
    '新品': 10, '上新': 10, '新款': 10, '新色': 10, '新设计': 10,
    '新出的': 10, '刚出': 9, '刚上': 9, '最新款': 11,
    '联名款': 12, '联名': 11, '合作款': 10,
    '限量款': 12, '限量版': 12, '限定款': 11, '限定色': 11, '限量': 9,
    '季节限定': 12, '季节款': 11, '季节色': 10,
    '今年流行': 11, '今年最火': 10, '今年流行色': 11,
    '今年款': 10, '这一季': 9, '这季': 9,
    '趋势色': 11, '流行色': 10, '流行款': 10,
    '潮流': 9, '时髦': 9, '新潮': 9,
    '爆款': 9,
    '今年最热': 10, '全网都在': 9,
    '复古': 8, '复古风': 8, '复古款': 8,
    'Y2K': 10, '多巴胺': 9, 'ins风': 9, '法式': 9, '日系': 8, '韩系': 8,
    '奶油感': 9, '奶咖色': 9, '大地色': 8, '莫兰迪': 9,
    '冷白': 8, '暖白': 8, '冷棕': 8, '暖棕': 8,
    '新工艺': 10, '升级版': 10, '改良版': 9, '加厚版': 8, '加宽版': 8,
}
...
async def _gen_person_frames(
    ...
    """
    reject_local_media("local person-frame extraction")
    if not boundaries:
        return {}
    try:
        import rembg
    ...
    async def _one(bi: int) -> tuple:
        ...
        try:
            ...
            # GPU rembg is mandatory; unavailable GPU means this optional overlay is skipped.
...
_GPU_WAIT_TIMEOUT = float(os.environ.get("GPU_WAIT_TIMEOUT", "600"))  # seconds to wait for remote GPU
...
async def _edit_via_gpu(
    ...
    payload = {
        ...
        "idempotency_key": f"clip:{room_id}:{mp4_filename}:{len(selected)}:{out_path}",
    ...
    async def _submit_aio() -> Optional[str]:
        ...
        if status_code == 404 and mp4_path and os.path.exists(mp4_path):
            ...
            logger.info("GPU file missing; uploading %s once before retry", mp4_filename)
            try:
...
# ── Retired local fallback boundary ──────────────────────────────────────────
...
async def _fast_local_clip(
    ...
    """Retired local encoder entry point; all encoding runs on remote GPU."""
