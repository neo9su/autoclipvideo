...
Create all tables and reset interrupted jobs."""
    with sqlite3.connect(DB_PATH) as conn:
        ...
        """)
        conn.execute(
            "UPDATE director_jobs SET status = 'error', error = 'service restarted' WHERE status IN ('queued', 'processing')"
        )
        conn.execute("""
...
def _load_director_jobs() -> dict:
    """Load persisted director jobs from DB into memory dict."""
    with sqlite3.connect(DB_PATH) as conn:
        conn.row_factory = sqlite3.Row
        rows = conn.execute("SELECT * FROM director_jobs").fetchall()
    return {r["job_id"]: dict(r) for r in rows}
...
def _aligned_segment_times(segment) -> tuple[float, float]:
    ...
    try:
        ...
        with open(srt_path, "w", encoding="utf-8") as f:
            for i, seg in enumerate(segments, 1):
                start, end = _aligned_segment_times(seg)
                f.write(f"{i}\n")
...
async def _has_audio_stream(path: str, timeout_s: int = 10) -> bool:
...
async def _run_ffmpeg(*cmd, timeout_s: int = 300) -> int:
    """Run ffmpeg with a hard timeout to prevent NVENC hangs from blocking the job queue."""
    proc = await asyncio.create_subprocess_exec(
        *cmd,
    ...
    try:
        ...
        _logging.getLogger(__name__).error(
            "ffmpeg timed out after %ds: %s", timeout_s, " ".join(cmd[:5]) + "..."
        )
        try:
            proc.kill()
            await proc.wait()
...
_director_jobs = _load_director_jobs()
...
async def _auto_cleanup_loop():
    ...
    while True:
        ...
        now = time.time()
        for store in (_jobs, _clip_jobs, _tts_jobs, _concat_jobs, _classic_concat_jobs, _director_jobs):
            expired = [
...
# ── Global exception handler to prevent crashes from killing the service ──────
@app.exception_handler(Exception)
async def global_exception_handler(request, exc):
    """Catch unhandled exceptions so a single job crash doesn't take down the service."""
    import traceback
...
async def health():
    ...
    director_processing = sum(1 for j in _director_jobs.values() if j.get("status") == "processing")
    director_queued = sum(1 for j in _director_jobs.values() if j.get("status") == "queued")
    director_pending = sum(1 for j in _director_jobs.values() if j.get("status") not in ("done", "error"))
    return {
        "status": "ok",
        "jobs": len(_jobs),
        ...
        "director_processing": director_processing,
...
def _update_director_job(job_id: str, **kwargs):
    ...
    # Persist to DB so jobs survive service restarts
    try:
        with sqlite3.connect(DB_PATH) as conn:
            ...
            conn.execute(
                f"INSERT INTO director_jobs (job_id, status, phase, pct, error, output_path, thumb_path, created_at) "
                f"VALUES (?, ?, ?, ?, ?, ?, ?, datetime('now')) "
                f"ON CONFLICT(job_id) DO UPDATE SET {vals}",
                params,
            )
            conn.commit()
    except Exception:
        pass  # DB write failure must not break the job
...
async def _nvenc_director_merge(seg_files_with_dur: list, out_dir: str,
    ...
    async def _has_video_stream(path: str, timeout_s: int = 10) -> bool:
        proc = await asyncio.create_subprocess_exec(
...
async def _do_director_job(job_id: str, clips: list, ass_content: str,
    ...
    try:
        ...
        for i, clip in enumerate(clips):
            ...
            # Phase 2: Comparison scene → split-screen effect (before/after)
            if scene_type == "comparison" and duration >= 4.0:
                # Create split-screen: left = original, right = enhanced (warm/bright)
                _SF_COMPARISON = (
                    f"scale={CLIP_W}:{CLIP_H}:force_original_aspect_ratio=decrease:flags=lanczos,"  # 1080x1920
                    f"pad=2*iw:{CLIP_H}:(ow-iw)/2:(oh-ih)/2,"  # double width, centered
                    f"split[original][enhanced],"  # split into two streams
                    # Left side: original (cool/natural)
                    f"[original]crop=iw/2:ih:0:0[left],"  # crop left half
                    # Right side: enhanced (warm/bright for comparison)
                    f"[enhanced]crop=iw/2:ih:iw/2:0[right],"  # crop right half
                    f"[right]eq=brightness=0.05:contrast=1.1:saturation=1.2[right_enh],"  # warm enhancement
                    # Side-by-side labels
                    f"[left]drawtext=text='Before':fontsize=48:fontcolor=white:x=20:y=20[left_l],"  # label
                    f"[right_enh]drawtext=text='After':fontsize=48:fontcolor=white:x=20:y=20[right_l],"  # label
                    # Merge side by side
                    f"[left_l][right_l]hstack[split],"
                    # Center the split-screen in the frame
                    f"[split]crop={CLIP_W}:{CLIP_H}:(ow-iw)/2+(ow-2*iw)/2:0[final]"
                )
                rc = await _run_ffmpeg(
                    "ffmpeg", "-y",
                    "-ss", f"{start:.3f}", "-t", f"{duration + 0.1:.3f}", "-i", mp4_path,
                    "-vf", f"trim=0:{duration:.3f},setpts=PTS-STARTPTS,{_SF_COMPARISON}",
                ...
                if rc != 0 or not os.path.exists(seg_out):
                    raise RuntimeError(f"Comparison clip {i} preprocess failed (rc={rc})")
                seg_files_with_dur.append((seg_out, duration))
                _update_director_job(job_id, pct=int((i + 1) / n * 40))
                continue
            
            # Normal clip preprocessing
            rc = await _run_ffmpeg(
                "ffmpeg", "-y",
                "-ss", f"{start:.3f}", "-t", f"{duration + 0.1:.3f}", "-i", mp4_path,
    ...
    if os.path.isdir(director_base):
        for entry in os.listdir(director_base):
            ...
            try:
                ...
                pass
        # Also clean up completed director jobs from DB
        try:
            with sqlite3.connect(DB_PATH) as conn:
                conn.execute(
                    "DELETE FROM director_jobs WHERE status IN ('done', 'error') "
                    "AND datetime('now', '-' || ? || ' seconds') > created_at",
                    (_JOB_TTL_SECONDS,),
                )
                conn.commit()
        except Exception:
