...
## v1.4.0 变更说明（2026-03-27）

### Bug 修复

#### 1. segment_merger.py — `list index out of range` 崩溃修复

**根因**：当分组内所有片段文件单独超过 `MERGE_MAX`（200MB）时，选片循环立即 `break` 导致 `selected = []`，随后 `selected[0][0]` 抛出 `IndexError`。

**修复**（`backend/segment_merger.py:386`）：
```python
# 之前
if len(selected) == 1:
# 之后
...
影响：分组合并不再因全超大文件而崩溃，会回退到逐文件处理。

---

#### 2. 批量排期"undefined 篇"Bug 修复

**根因**：FastAPI 路由匹配顺序问题 —— `GET /api/publish-tasks/unscheduled-groups` 和 `POST /api/publish-tasks/batch-schedule` 声明在 `GET /api/publish-tasks/{task_id}` **之后**，导致请求被动态路由匹配（`task_id="unscheduled-groups"`），返回 422 校验错误而非正常数据。前端收到 `{"detail": [...]}` 数组，`.length` 为字段数而非分组数，显示"undefined 篇"。

**修复**（`backend/main.py`）：将两个特定路由**移到** `/{task_id}` 路由声明**之前**，并删除重复定义。

---

#### 3. GPU 服务 UNIQUE 约束错误修复

**根因**：GPU 服务重启后，之前处于 `processing` 状态的作业记录仍在 jobs.db 中，重新提交同一作业时 `INSERT` 触发 `UNIQUE constraint failed`，使转录任务无法恢复。

**修复**（`gpu_service/main.py:93`）：
```python
# 之前
"INSERT INTO jobs (...) VALUES (?, ?, ?, 'processing')"
# 之后
"INSERT OR REPLACE INTO jobs (...) VALUES (?, ?, ?, 'processing')"
```

> 注意：本次修改通过本机 `scp` 部署，绕过 PowerShell UTF-8 编码损坏问题（PowerShell `Set-Content -Encoding UTF8` 会破坏中文多字节字符）。

---

### 新功能：GPU 恢复自动唤醒转录队列

**背景**：GPU 服务器离线后，`gpu_state` watcher 的指数退避增长到最大 300 秒，导致后端轮询长时间卡在 `wait_until_online()`，即使 GPU 已恢复在线也无法及时处理队列中的作业。

**修复**（`backend/main.py`，`_on_gpu_online` 回调）：
```python
async def _on_gpu_online():
    ...
    finally:
        # GPU 恢复上线时立即唤醒转录轮询，无需等待下一个轮询周期
        await flush_poll()
```

效果：GPU 上线后 `asyncio.Event` 被立即 set，转录 poll 循环无需等待即可开始派发积压作业，彻底防止"GPU空闲但队列挂起"的情况。

---

### 新增文档：巨量千川优质/低质短视频标准

新增 `DOUYIN_VIDEO_QUALITY_GUIDE.md`，整合巨量千川官方文档（6份）内容：

| 章节 | 内容 |
|------|------|
| 优质素材价值与激励 | 流量激励、自然流量、交易收益三维度说明 |
| 优质判定标准 | 声画质量（硬性门槛）+ 信息价值 + 互动表现 |
| 美妆/假发行业特征 | 产品展示为主、人物口播、重号召商品、弱营销元素 |
| 低质素材避坑手册 | 夸大宣传11种 + 引人不适7种 + 内容低质3种 + 素材低质5种 |
| AI元数据生成原则 | 专针对假发行业的标题/描述/标签生成禁忌与规范 |
| 发布前自查清单 | 声画质量 + 内容合规 + 制作质量三维度逐项确认 |

README 同步更新：目录结构新增文件条目，"自动发布"章节新增规范说明链接。
...
| 版本 | 日期 | 主要变更 |
|------|------|---------|
| v1.4 | 2026-03-27 | segment_merger崩溃修复；批量排期路由修复；GPU UNIQUE约束修复；GPU恢复自动唤醒队列；新增巨量千川优质素材规范文档 |
| v1.3 | 2026-03-27 | GPU服务器 Python 3.13→3.11 根治 BSOD；watchdog/start_all.bat 同步更新 |
| v1.2 | 2026-03-26 | GPU服务编码修复+compute_type优化；发布页商品筛选修复；分组自动刷新；增强作业卡死修复；AI文案多样性提升 |
| v1.1 | 2026-03-25 | 画质过滤+流程优化：低画质跳过、时长过短跳过、直播流分辨率监控、AI文案4方案多标签、封面暖色主题、发布分组自动合并 |
| v1.0 | 2026-03-25 | 直播流质量升级：webcast API原画流；fragmented MP4防损坏 |
...
| GPU服务 | 在线 healthy |
| 批量排期 | 已修复（显示正确分组数量） |
| 转录队列 | GPU恢复后自动唤醒（flush_poll） |
| segment_merger | list index out of range 已修复 |
| 文档 | DOUYIN_VIDEO_QUALITY_GUIDE.md 新增 |

## 整点快照 2026-06-24T04:01 UTC

- pending_transcribe: 46
- gpu_busy: False
- queue_depth: 0
- last_submit_at: 
- issues: 队列卡住(pending=46, 40min无提交); Watchdog不可达


## 整点快照 2026-06-24T05:01 UTC

- pending_transcribe: 44
- gpu_busy: False
- queue_depth: 0
- last_submit_at: 
- issues: 队列卡住(pending=44, 100min无提交); Watchdog不可达


## 整点快照 2026-06-24T06:01 UTC
...
## 整点快照 2026-06-24T14:02 UTC

- pending_transcribe: 3
- gpu_busy: False
- queue_depth: 0
- last_submit_at: 
- issues: 队列卡住(pending=3, 641min无提交); Watchdog不可达


## 整点快照 2026-06-24T15:02 UTC

- pending_transcribe: 10
- gpu_busy: False
- queue_depth: 0
- last_submit_at: 
- issues: 队列卡住(pending=10, 701min无提交); Watchdog不可达


## 整点快照 2026-06-24T16:02 UTC

- pending_transcribe: 23
- gpu_busy: False
- queue_depth: 0
- last_submit_at: 
- issues: 队列卡住(pending=23, 761min无提交); Watchdog不可达


## 整点快照 2026-06-24T17:02 UTC

- pending_transcribe: 29
- gpu_busy: False
- queue_depth: 0
- last_submit_at: 
- issues: 队列卡住(pending=29, 822min无提交)


## 整点快照 2026-06-24T18:03 UTC
...
## 整点快照 2026-06-24T19:03 UTC

- pending_transcribe: 10
- gpu_busy: False
- queue_depth: 0
- last_submit_at: 
- issues: 队列卡住(pending=10, 942min无提交); Watchdog不可达


## 整点快照 2026-06-24T20:03 UTC
...
## 整点快照 2026-06-25T14:05 UTC

- pending_transcribe: 3
- gpu_busy: False
- queue_depth: 0
- last_submit_at: 
- issues: 队列卡住(pending=3, 2084min无提交)


## 整点快照 2026-06-25T15:00 UTC

- pending_transcribe: 4
- gpu_busy: False
- queue_depth: 0
- last_submit_at: 
- issues: 队列卡住(pending=4, 2140min无提交)


## 整点快照 2026-06-25T15:05 UTC

- pending_transcribe: 1
- gpu_busy: False
- queue_depth: 0
- last_submit_at: 
- issues: 队列卡住(pending=1, 2145min无提交)


## 整点快照 2026-06-25T16:01 UTC

- pending_transcribe: 8
- gpu_busy: False
- queue_depth: 0
- last_submit_at: 
- issues: 队列卡住(pending=8, 2200min无提交)


## 整点快照 2026-06-25T17:01 UTC

- pending_transcribe: 1
- gpu_busy: False
- queue_depth: 0
- last_submit_at: 
- issues: 队列卡住(pending=1, 2260min无提交)
